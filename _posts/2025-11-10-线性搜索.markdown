...
> **CAUTION** 因为转换为关于步长的函数，所以精确线性搜索算法是用于**处理一维函数**的，并且**计算量很大**。
{: .prompt-danger }
...
> **NOTE** 注意，在这里，随着迭代次数的增加，加倍系数$t$是呈指数增长。
{: .prompt-info }
...
> **NOTE** 我们会发现$\lambda_k$和$\mu_k$关于$a_k+\frac{1}{2}(b_k-a_k)$对称。
{: .prompt-info }
...
> **NOTE** 我们会发现在$k+1$次的试探点选取时，试探点与$\lambda_k$无关。
{: .prompt-info }
...
> **NOTE** 此时的对称轴为$a_{k}+\frac{1}{2}\tau(b_k-a_k)$
{: .prompt-info }
...
> **CAUTION** 注意，该算法只适用于单峰函数，若是一个函数有多个极小值，需要进行分割到每段内进行计算。
{: .prompt-danger }
...
> **NOTE** 因为是过点$(0,f(x_k))$的直线并且其斜率通过$f(x_k)$的视角，当时并没有产生步长增量，所以计算斜率时$\alpha=0$。
{: .prompt-info }
